...
export async function generateStaticParams() {
  ...
  return currentProjects.map((p) => ({ slug: p.slug }))
}
...
export async function generateMetadata({ params }: { params: { slug: string } }) {
  ...
  return {
    ...
    description:
      project.description ||
      `${project.name} — a multifamily development underway by Mission Properties in ${project.location}.`,
  }
}

export default async function CurrentProjectPage({ params }: { params: { slug: string } }) {
  const { slug } = params
  const currentProjects = await getCurrentProjects()
  const project = currentProjects.find((p) => p.slug === slug)
  if (!project) notFound()
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <div>
              ...
              <p className="font-sans text-sm" style={{ color: 'rgba(244,239,230,0.45)' }}>
                Browse our active developments and completed portfolio.
              </p>
            </div>
            <div className="flex gap-4 shrink-0">
              <Link href="/current-projects" className="btn-primary">All Current</Link>
              <Link
